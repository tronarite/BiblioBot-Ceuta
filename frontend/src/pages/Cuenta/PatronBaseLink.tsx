...
export function PatronBaseLink() {
  ...
  return (
    <div className="rounded-xl border border-slate-200 bg-white p-5">
      <h2 className="mb-3 font-medium text-slate-800">Cuenta PatronBase</h2>

      {status.estadoVinculacion === "vinculada" ? (
        <div className="flex items-center justify-between">
          <div>
            <span className="rounded-full bg-emerald-100 px-2 py-0.5 text-xs font-medium text-emerald-700">Vinculada</span>
            <p className="mt-2 text-sm text-slate-600">{status.patronbaseEmail}</p>
          </div>
          <button onClick={desvincular} disabled={busy} className="rounded-lg border border-red-200 px-3 py-1.5 text-xs font-medium text-red-600 hover:bg-red-50">
            Desvincular
          </button>
        </div>
      ) : (
