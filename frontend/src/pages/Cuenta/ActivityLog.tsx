...
const ESTILOS: Record<string, string> = {
  reserva_exitosa: "bg-emerald-100 text-emerald-700",
  reserva_fallida: "bg-red-100 text-red-700",
  reintento: "bg-amber-100 text-amber-700",
  puesto_retirado: "bg-red-100 text-red-700",
...
export function ActivityLog() {
  ...
  return (
    <div className="rounded-xl border border-slate-200 bg-white p-5">
      <h2 className="mb-3 font-medium text-slate-800">Actividad</h2>
      {items.length === 0 ? (
          ...
          {items.map((item) => (
            <li key={item.id} className="flex items-start gap-3 text-sm">
              ...
              <div>
                ...
                <p className="text-xs text-slate-400">{new Date(item.fecha).toLocaleString("es-ES")}</p>
              </div>
            </li>
          ))}
        </ul>
